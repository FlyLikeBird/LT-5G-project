...
import { findMaxAndMin } from '@/utils/array';
...
function CompareLineChart({ theme, xData, typeRule, yData, y2Data, info, startDate, timeType, currentAttr, dispatch }){
                ...
                option={{
                    legend:[
                        ...
                        },
                        {
                            ...
                            formatter:name=>{
                                let temp = findMaxAndMin( name === info.title ? yData : y2Data ,false );
                                ...
                                return `
                                    ...
                                    {value|最大值:}{num|${temp.max && temp.max.value ? temp.max.value : '--'}}{num|${maxTime || '--'}}\n
                                    {value|最小值:}{num|${temp.min && temp.min.value ? temp.min.value : '--'}}{num|${minTime || '--'}}\n
...
function areEqual(prevProps, nextProps){
    if ( prevProps.yData !== nextProps.yData || prevProps.typeRule !== nextProps.typeRule ||  prevProps.theme !== nextProps.theme  ) {
        return false;
    } else {
        return true;
    }
}
export default React.memo(CompareLineChart, areEqual);