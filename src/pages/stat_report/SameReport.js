...
function MeterReport({ dispatch, user, fields, report }){
    const { authorized, currentMenu, theme } = user;
    const { allFields, currentField, currentAttr, allFieldAttrs, currentEnergy, expandedKeys, treeLoading } = fields;
    const { isLoading, list, mark, currentPage } = report;
    useEffect(()=>{
        if ( authorized ) {
            dispatch({ type:'report/initAdjoinReport', payload:{ mark:currentMenu.menuCode === 'same_report' ? 'yoy' : 'chain' }});
        }
    },[authorized]);
            ...
            children:(           
                ...
                <Tabs
                    ...
                    onChange={activeKey=>{
                        ...
                        .then((fieldAttrs)=>{
                            dispatch({ type:'report/fetchAdjoinReport'})
                        })
                    }}
    ...
    let columns = [
        ...
        { title:'本期用电量(kwh)', dataIndex:'nowSumUsage' },
        { title:'上期用电量(kwh)', dataIndex:'lastSumUsage' },
        { title:'增加量(kwh)', dataIndex:'riseQuantity' },
        { title: mark === 'yoy' ? '同比' : '环比', dataIndex:'ratio', render:value=>(<span>{ ( value || '--' ) + ( value ? '%' : '' )}</span>) }
    ];
    const content = (     
        <div style={{ position:'relative' }}>
            ...
            <div style={{ height:'40px', display:'flex', justifyContent:'space-between' }}>
                <CustomDatePicker onDispatch={()=>dispatch({ type:'report/fetchAdjoinReport'})} />
                <Button type='primary' onClick={()=>{
                    if ( list.length ) {
                        let fileTitle =  mark === 'yoy' ? '同比报表' : '环比报表';
                        var aoa = [], thead = [];
